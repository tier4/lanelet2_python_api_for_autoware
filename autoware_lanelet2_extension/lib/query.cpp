  ...
  // find by angle
  {
    ...
    for (const auto & llt : candidate_lanelets) {
      ...
      if (!segment.empty()) {
        ...
        angle_diff = std::abs(lanelet::utils::normalize_radian(segment_angle - pose_yaw));
      }
  ...
  ConstLanelet * closest_lanelet_ptr, const double dist_threshold, const double yaw_threshold)
{
  ...
  // find closest lanelet within yaw_threshold
  {
    ...
    for (const auto & llt_pair : candidate_lanelets) {
      ...
      double lanelet_angle = getLaneletAngle(llt_pair.first, search_pose);
      double angle_diff = std::abs(lanelet::utils::normalize_radian(lanelet_angle - pose_yaw));
