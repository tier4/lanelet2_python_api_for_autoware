...
def test_utility_utilities(lanelet_map, routing_graph):
    ...
    search_pose = np.array([3685.0, 73750.0, 0, 0], dtype=np.double) #[x, y, z, yaw]
    ...
    closest_segment = utilities.getClosestSegment(
        lanelet2.core.BasicPoint2d(x=search_pose[0], y=search_pose[1]), #x,y
    ...
    ax.scatter([search_pose[0]], [search_pose[1]], marker="o", label="search_pose")

    print(f"""{utilities.getLaneletAngle(closest_lanelet, search_pose[0:3])=}""") #x, y, z
    print(f"""{utilities.isInLanelet(search_pose, closest_lanelet)=}""")
    print(f"""{utilities.isInLanelet(search_pose, closest_lanelet, 5.0)=}""")
    closest_center_pose = utilities.getClosestCenterPose(closest_lanelet, search_pose[0:3])
    ax.scatter(
        [closest_center_pose[0]],
        [closest_center_pose[1]],
        marker="o",
...
def test_utility_query(lanelet_map, routing_graph):
    ...
    search_point = np.array(
        [lanelet56_centerline_center.x, lanelet56_centerline_center.y, 0.0], dtype=np.double)
    ...
    search_pose = np.zeros(4)
    search_pose[0:3] = search_point
    print(
...
if __name__ == "__main__":
    ...
    lanelet_map = test_io("/home/ukenryu/Downloads/sample-map-planning/lanelet2_map.osm", proj)
